...
def step_impl(context, browser, url):
    if ("chrome" or "Chrome" or "CHROME") in browser:
        ...
        allure.attach.file("Chrome_page", chrome_page_obj.save_chrome_web_page_screenshot(),
...
def step_impl(context, title, no_links):
    ...
    allure.attach.file("Chrome_" + title, context.chrome_page_obj.save_chrome_web_page_screenshot(),
                       attachment_type=AttachmentType.PNG)
