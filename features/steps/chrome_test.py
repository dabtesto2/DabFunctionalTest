...
def step_impl(context, browser, url):
    if ("chrome" or "Chrome" or "CHROME") in browser:
        ...
        allure.attach.file(chrome_page_obj.save_chrome_web_page_screenshot(), name="Chrome_page",
...
def step_impl(context, title, no_links):
    ...
    allure.attach.file(context.chrome_page_obj.save_chrome_web_page_screenshot(), name="Chrome_" + title,
                       attachment_type=AttachmentType.PNG)
